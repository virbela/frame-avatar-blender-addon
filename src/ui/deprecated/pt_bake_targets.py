            ...
            if HT.selected_bake_target != -1:
                ...
                if obj := HT.avatar_mesh:
                    if obj.data.shape_keys:
                        self.layout.prop_search(
                            et, "shape_key_name", obj.data.shape_keys, "key_blocks"
                        )
                ...
                if et.bake_mode == "UV_BM_REGULAR":
                    ...
                    if len(et.variant_collection):
                        ...
                        if variant.workmesh:
                            self.layout.prop_search(
                                variant, "uv_map", variant.workmesh.data, "uv_layers"
                            )
                        else:
                            self.layout.label(
                                text="Select work mesh to choose UV map", icon="ERROR"
                            )
                        ...
                        if bake_scene := require_bake_scene():
                            self.layout.prop_search(
                                variant, "workmesh", bake_scene, "objects"
                            )
                        else:
                    ...
                    raise InternalError(
                        f"et.bake_mode set to unsupported value {et.bake_mode}"
                    )
...
class FABA_UL_bake_variants(UIList):
    def draw_item(
        ...
        if item.image:
            ...
            layout.prop(
                item, "name", icon="UNLINKED", text="", emboss=False, translate=False
            )
...
class FABA_UL_bake_targets(UIList):
    def draw_item(
        ...
        if item.bake_mode == "UV_BM_MIRRORED":
            if target_item := data.bake_target_collection[item.mirror_source]:
                ...
                layout.prop(
                    item,
                    "name",
                    icon=UV_ISLAND_MODES.members[target_item.uv_mode].icon,
                    text="",
                    emboss=False,
                    translate=False,
                )
        ...
        layout.prop(
            item,
            "name",
            icon=UV_ISLAND_MODES.members[item.uv_mode].icon,
            text="",
            emboss=False,
            translate=False,
        )
            ...
            if item.target < len(ht.bake_target_collection):
                if target := ht.bake_target_collection[item.target]:
                    layout.prop(
                        target,
                        "name",
                        icon=UV_ISLAND_MODES.members[target.uv_mode].icon,
                        text="",
                        emboss=False,
                        translate=False,
                    )
    ...
    def draw_item(
        ...
        if ht := get_homeomorphic_tool_state(context):
            ...
            if primary and secondary:
                row.label(
                    text=primary.name,
                    icon=UV_ISLAND_MODES.members[primary.uv_mode].icon,
                )
